...
vector<string> MessageForger::splitMessages(string msgstr)
{
    string pattern("} \n{");
    vector<string> listMessages;
    int index;

    while((index = msgstr.find(pattern)) >= 0){
        string msg = msgstr.substr(0, index + 1);
        listMessages.push_back(msg);
        msgstr = msgstr.substr(index + 3);
    }
    listMessages.push_back(msgstr);
    return listMessages;
}
